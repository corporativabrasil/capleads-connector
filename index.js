...
async function iniciarWhatsApp() {
  ...
  const { state, saveCreds } =
    await useMultiFileAuthState("/app/auth_info"); // 👈 VOLUME RAILWAY

  sock = makeWASocket({
    auth: state,
    browser: ["CapLeads", "Chrome", "1.0"],
    printQRInTerminal: true, // 👈 OBRIGATÓRIO NO RAILWAY
  ...
  sock.ev.on("connection.update", (update) => {
    ...
    if (connection === "close") {
      ...
      if (code === DisconnectReason.loggedOut) {
        console.log("❌ Sessão expirada. Novo QR necessário.");
      } else {
  ...
  sock.ev.on("messages.upsert", async ({ messages }) => {
    ...
    console.log("📩 Mensagem:", numero, texto);
...
// testeimport express from "express";
import cors from "cors";
import makeWASocket, {
  DisconnectReason,
  useMultiFileAuthState
